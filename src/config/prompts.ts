...
5. 【格式要求】：只输出【原文】和${label}两部分的纯文字内容，不要输出任何格式说明或排版指令。${nicknameSuffix}`
    ...
    case 'story':
      return `你是一位温柔而有创意的睡前故事创作者。${r}正准备给未出生的${baby}朗读你写的故事。
...
7. 【多样性】：每次的故事风格都要有变化。不要陷入相同的叙事套路。
8. 【禁止】：故事正文中不要出现具体的日期或时间。${nicknameSuffix}`
...
/** 故事类型 — 唯一的随机维度，控制整体风格走向，角色/场景/情节全交给 AI */
const STORY_TYPES = [
...
export function getUserPrompt(mode: ContentMode, settings: UserSettings): string {
  ...
  switch (mode) {
    ...
    case 'story':
      return `（请求编号：${date}-${Math.random().toString(36).slice(2, 6)}，仅用于去重，不要在故事中出现）
故事风格：${pickRandom(STORY_TYPES)}
请自由创作，角色、场景、情节都由你决定。写一个和以往不一样的故事。${pregCtx}`
    case 'babyInfo':
